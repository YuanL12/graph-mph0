...
import abmph
from abMPH import compute_absolute_MPH0_multi_critical_filtration, compute_grid_H0
...
def load_sydney_urban_objects(labels_dict,
        folder_path = "./sydney-urban-objects-dataset/objects"):
...
if __name__ == "__main__":
    ...
    for i, file_path in enumerate(files_path_list):
        ...
        grid_size = 100
        H0_grid = compute_grid_H0(grid_size, x_range, y_range, betti0s, betti1s, betti2s)
        ...
        if not os.path.exists(f"./sydney-urban-objects-dataset/H0_grids_sample_{sample_size}"):
            os.makedirs(f"./sydney-urban-objects-dataset/H0_grids_sample_{sample_size}")

        np.save(f"./sydney-urban-objects-dataset/H0_grids_sample_{sample_size}/{bin_file_name}.npy", H0_grid_masked)
