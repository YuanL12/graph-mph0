...
def point_cloud_to_1_critical_filtration_fast(points, x_y_swapped=False):
    ...
    neg_degrees = -degrees
    filt_func_vertices = np.column_stack(
        [sorted_dists.ravel(), np.tile(neg_degrees, n)]
    )
    ...
    edges = np.column_stack(
        [
            np.repeat(base, n - 1) + np.tile(np.arange(n - 1), n),
    ...
    filt_func_edges = np.column_stack(
        [sorted_dists[:, 1:].ravel(), np.tile(neg_degrees[:-1], n)]
    )
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if idx < n:
                ...
                filt_func_pair.append(
                    np.column_stack((max_r[valid_range], -degrees[valid_range]))
                )
...
def point_cloud_to_1_critical_filtration(points, x_y_swapped=False):
    ...
    inds = -np.arange(0, n)
    filt_func_edges = [
        np.column_stack([sorted_edge_lengths[i][1:], inds[:-1]]) for i in range(n)
    ]
        ...
        for j in range(i + 1, n):
            ...
            for index, r in enumerate(max_rs):
                if r >= D[i, j]:
                    ...
                    filt_func_edges_pair_vertices.append(
                        [max_rs[index], -degrees[index]]
                    )
...
def compute_abs_mph0(vertices, edges, node_features, edge_features):
    ...
    mph_g = graph_mph.Graph(vertices, node_features, edges, edge_features)
...
def compute_abs_mph0_inplace(vertices, edges, node_features, edge_features):
    ...
    # Create the Graph object
    mph_g = graph_mph.Graph(vertices, node_features, edges, edge_features)
    res = graph_mph.compute_MPH0_DTree_Inplace(mph_g)
    return res
...
def compute_betti0_betti1_vertex(pd_matrix: np.ndarray) -> tuple:
    ...
    inds = -np.arange(0, n)
    betti0s_v = [
        np.column_stack([sorted_edge_lengths[i][0:], inds]) for i in range(n)
    ]  # vertex i
    betti1s_v = [
        np.column_stack([sorted_edge_lengths[i][1:], inds[:-1]]) for i in range(n)
    ]
