...
struct ExactValue {
    ...
    mutable std::vector<unsigned> indexes;  // indexes of points corresponding to this value (e.g.
                                            // points whose birth time is this value)
    ...
    ExactValue(exact e) : exact_value(e) {
        if (denominator(e) == 0) {
            ...
            double_value = numerator(e).convert_to<double>() /
    ...
    bool operator<=(const ExactValue& other) const {
        ...
        if (almost_equal(double_value, other.double_value)) return exact_value <= other.exact_value;
...
inline double ExactValue::epsilon = std::pow(2, -30);
