...
It uses rational numbers from boost::multiprecision to deal with floating point
precision.

Although it is not very nessary for this project as we do not need projection on
lines, we keep it here for comparison using the same data type.
*/
namespace rivet {
...
exact approx(double x) {
    ...
    if (log >= d) return exact((int)floor(x));
...
// exact value struct
struct ExactValue {
    double double_value = std::pow(2, -30);
    exact exact_value;

    mutable std::vector<unsigned>
        indexes;  // indexes of points corresponding to this value (e.g. points
                  // whose birth time is this value)

    static double epsilon;

    ExactValue(exact e) : exact_value(e) {
        double_value =
            numerator(e).convert_to<double>() /
            denominator(e).convert_to<double>();  // can aos use static_cast in C++11
    }
    ...
    static bool almost_equal(const double a, const double b) {
        ...
        if (diff <= (std::abs(a) + std::abs(b)) * epsilon) return true;
        return false;
    }
};
