...
class R2 {
   public:
    ...
    double operator[](int index) const {
        switch (index) {
            ...
            case 1:
                return y;
            default:
                throw std::out_of_range("Index out of range for R2");
        }
    }
    ...
    bool operator>(const R2& other) const { return other < *this; }

    bool operator==(const R2& other) const { return x == other.x && y == other.y; }

    bool operator!=(const R2& other) const { return !(*this == other); }

    bool operator<=(const R2& other) const { return *this < other || *this == other; }

    // minus operator
    R2 operator-(const R2& other) const { return R2(x - other.x, y - other.y); }
