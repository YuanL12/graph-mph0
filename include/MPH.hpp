...
void local_collapse_edges_Grade_Version(GGraph& g) {
    ...
    std::vector<size_t> vertex_dict;
    vertex_dict.resize(g.get_nvertices());
    ...
    for (size_t v_idx = 0; v_idx < g.get_nvertices(); ++v_idx) {
        ...
        if (v.get_id() == -1) {
            visited[v_idx] = true;
            continue;
        }  // if the vertex is removed, skip it
        ...
        while (!stack.empty()) {
            ...
            if (!visited[u.get_id()] && u.get_id() != -1) {
                ...
                for (const auto& eid_local : g.get_adj(u.get_id())) {
                    ...
                    GVertex x = (e.get_v0() == u.get_id()) ? g.get_gvertex(e.get_v1())
...
void collapse_to_vertex_minimal_Grade_Version(GGraph& g) {
    ...
    for (size_t v_idx = 0; v_idx < g.get_nvertices(); ++v_idx) {
        ...
        if (!check_minimal) {
            continue;
        }  // if v is not minimal, skip to the next vertex
        ...
        while (!stack.empty()) {
            ...
            if (!visited[u.get_id()] && u.get_id() != -1) {
                visited[u.get_id()] = true;  // despite we have duplicate edges after
                                             // merge, it is visited only once
                if (e != GEdge::NULL_GEDGE) {
                    vertex_dict[u.get_id()] = v.get_id();
                    g.remove_edge(e.get_id());
                }
...
compute_MPH0(GGraph& g) {
    ...
    std::sort(grades_collection.begin(), grades_collection.end(),
              [&g](std::tuple<int, int> x, std::tuple<int, int> y) {
                  const GradePoint& gx = std::get<0>(x) == 0
                                             ? g.get_gvertex(std::get<1>(x)).get_grade()
                                             : g.get_gedge(std::get<1>(x)).get_grade();
                  const GradePoint& gy = std::get<0>(y) == 0
                                             ? g.get_gvertex(std::get<1>(y)).get_grade()
                                             : g.get_gedge(std::get<1>(y)).get_grade();
                  return GradePoint::lexicographical_less_than(gx, gy) ||
                         (gx == gy && std::get<0>(x) < std::get<0>(y));
              });
